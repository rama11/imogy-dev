...
<footer class="main-footer">
	...
	<strong>Copyright &copy; {{date('Y')}} <a href="https://www.sinergy.co.id">PT. Sinergy Informasi Pratama</a>.</strong> All rights
	reserved.
</footer>
@endsection 
@section('script')
...
<script>
		...
		if($("#escalateRCA").val() == "" && $("#escalateNameEngineer").val() == "" && $("#escalateContactEngineer").val() == ""){
			...
			swalWithCustomClass.fire({
				...
				if (result.isConfirmed) {
					$(".help-block").hide()
					$.ajax({
						url:"{{url('tisygy/mail/getEscalateMailTemplate')}}",
						type:"GET",
						success: function (result){
	...
	function sendEscalateEmail(){
		if($("#emailEscalateTo").val() == ""){
			...
			var typeAjax = "GET"
			var urlAjax = "{{url('tisygy/mail/sendEmailEscalate')}}"
			var dataAjax = {
				id_ticket:$("#ticketID").val(),
				contactEngineer:$("#escalateContactEngineer").val(),
				nameEngineer:$("#escalateNameEngineer").val(),
						...
						success: function(result){
							...
							if(result == 0){
								...
								swalWithCustomClass.fire({
									...
									text: "You can get your file now",
									type: 'success',
									confirmButtonText: '<a style="color:#fff;" href="report/bayu/' + result.slice(1) + '">Get Report</a>',
								})
							}
						}
					})
